...
def apply_cvar_risk_management(returns, cvar_threshold, window=252):
    ...
    risk_management_exit = cvar_series > cvar_threshold
    managed_returns = returns.copy()
    managed_returns[risk_management_exit] = 0
...
def apply_long_short_cash_strategy(returns, state_probs, cash_threshold, leverage):
    ...
    market_regime = np.where(
        state_probs.iloc[:, 0] > (1 - cash_threshold), 0,
        np.where(state_probs.iloc[:, 0] < cash_threshold, 1, 2)
    )
    
    strategy_returns = np.where(
        market_regime == 0, returns * leverage,
        np.where(market_regime == 1, -returns * leverage, 0)
    )
    
    return pd.Series(strategy_returns, index=common_index)

def train_and_save_hmm(train_data):
    np.random.seed(42)
...
logo = Image.open("Olympe Financial group (Logo) (1).png")
...
if gspc_data.empty:
    ...
    if hmm_model is None:
        hmm_model = train_and_save_hmm(train_data)
    ...
    current_state_prob = state_probs.iloc[-1, 0]
    current_probs = state_probs.iloc[-1]
